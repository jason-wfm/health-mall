...
import com.wechuang.mallshop.account.dao.UserCardDao;
 ...
 */
@Repository
public class UserCardRepositoryImpl extends BaseRepositoryImpl<UserCardDao, UserCard> implements UserCardRepository {

    @Override
    public Map<Integer, UserCard> getCardInfoMap(List<Integer> userIds) {
        List<UserCard> userCards = gets(userIds);
        Map<Integer, UserCard> userCardMap = new HashMap<>();

        if (CollectionUtil.isNotEmpty(userCards)) {
