...
import com.wechuang.mallshop.admin.repository.MenuBaseRepository;
...
import com.wechuang.mallshop.common.consts.*;
import com.wechuang.mallshop.common.exception.BusinessException;
import com.wechuang.mallshop.common.security.JwtSubject;
import com.wechuang.mallshop.common.security.JwtUtil;
...
import com.wechuang.mallshop.trade.model.input.OrderNumInput;
...
public class LoginServiceImpl implements LoginService {
    ...
    @Autowired
    private com.wechuang.mallshop.shop.repository.StoreBaseRepository storeBaseRepository;
    ...
    public LoginRes login(UserBase user) {
        ...
        //生成token
        // 签发token [healthmall-ext] 按真实身份签发（原硬编码 roleId=9/siteId/storeId/chainId=0）
        Integer tokenRoleId = user.getRoleId() != null ? user.getRoleId() : 0;
        Integer tokenSiteId = user.getSiteId() != null ? user.getSiteId() : 0;
        Integer tokenStoreId = user.getStoreId() != null ? user.getStoreId() : 0;
        Integer tokenChainId = user.getChainId() != null ? user.getChainId() : 0;
        Integer tokenMerchantId = user.getMerchantId() != null ? user.getMerchantId() : 0;
        Integer tokenClientId = user.getClientId() != null ? user.getClientId() : 0;
        String access_token = JwtUtil.buildToken(
                new JwtSubject(user.getUserId(), user.getUserSalt(), tokenRoleId, tokenSiteId,
                        tokenStoreId, tokenChainId, tokenMerchantId, tokenClientId),
    ...
    public LoginRes login(Integer userId) {
        // [healthmall-ext] 改走 getByUserId 完成身份装配（get 仅查基础行，不含 roleId/storeId/merchantId）
        UserBase user = getByUserId(userId);
    ...
    public LoginRes login(LoginReq loginReq) {
        ...
        if (loginReq.getIsManage()) {
            UserAdmin userAdmin = userAdminRepository.get(user.getUserId());

            if (userAdmin == null || !userAdmin.getUserEnable()) {
                throw new BusinessException(ConstantMsg.CODE_BUSINESS_VALIDATION_FAILED, __("没有相关权限"));
            }
            // [healthmall-ext] 管理端登录前装配身份字段，供签发真实 claims
            user.setRoleId(userAdmin.getRoleId());
            user.setSiteId(userAdmin.getSubsiteId() != null ? userAdmin.getSubsiteId() : 0);
            user.setStoreId(userAdmin.getStoreId() != null ? userAdmin.getStoreId() : 0);
            user.setChainId(userAdmin.getChainId() != null ? userAdmin.getChainId() : 0);
            user.setMerchantId(userAdmin.getMerchantId() != null ? userAdmin.getMerchantId() : 0);
        }
        // 分销员判断
        Integer sourceUserId = getSourceUserId();

        if (CheckUtil.isNotEmpty(sourceUserId)) {
            UserBase UserBase = userBaseRepository.get(sourceUserId);

            if (UserBase != null) {
                addSourceUserId(user.getUserId(), sourceUserId, 0, false);
            }
        }
    ...
    public Integer register(RegInput input) {
        ...
        if(input.getUserAvatar() == null){
            userInfo.setUserAvatar(configBaseService.getConfig("user_no_avatar", ""));
        }
        else
        {
        ...
        if (CheckUtil.isNotEmpty(input.getIndustryId())) {
            userInfo.setIndustryId(input.getIndustryId());
        }
        ...
        if (input.getRoleId().intValue() != ConstantRole.ROLE_USER) {
            ...
            if (input.getRoleId().intValue() == ConstantRole.ROLE_ADMIN) {
                ...
                //入驻角色可配置(seller_role_id,缺省沿用原硬编码1002),与 StoreBaseServiceImpl.editStateId 口径一致
            ...
            } else if (input.getRoleId().intValue() == ConstantRole.ROLE_SITE) {
                userAdmin.setUserRoleId(1004);
            }
        ...
        if (CheckUtil.isNotEmpty(userParentId)) {
            ...
            if (UserBase != null) {
                addSourceUserId(userId, userParentId, input.getActivityId(), true);
            }
    ...
    public UserBase getByUserId(Integer userId) {
        ...
        if (!ObjectUtil.isEmpty(userInfo)) {
            ...
            user.setIndustryId(userInfo.getIndustryId());
        }
        ...
        if (ObjectUtil.isNotEmpty(userAdmin)) {
            //角色编号:0-用户;2-商家;3-门店;8-租户;9-平台;
            user.setRoleId(userAdmin.getRoleId());
            user.setSiteId(userAdmin.getSubsiteId());
            user.setStoreId(userAdmin.getStoreId());
            user.setChainId(userAdmin.getChainId());

            // [healthmall-ext] 商家归属：商家角色直取；门店角色经店铺归属推导（spec §5.2）
            user.setMerchantId(userAdmin.getMerchantId() != null ? userAdmin.getMerchantId() : 0);
            if (userAdmin.getRoleId() != null && userAdmin.getRoleId() == ConstantRole.ROLE_CHAIN
                    && CheckUtil.isNotEmpty(userAdmin.getStoreId())
                    && (user.getMerchantId() == null || user.getMerchantId() == 0)) {
                StoreBase store = storeBaseRepository.get(userAdmin.getStoreId());
                user.setMerchantId(store != null && store.getMerchantId() != null ? store.getMerchantId() : 0);
            }

            user.setUserEnable(userAdmin.getUserEnable());
    ...
    public UserInfoOutput getInfoByUserId(Integer userId) {
        ...
        if (userInfo != null) {
            ...
            if (CheckUtil.isNotEmpty(userInfo.getUserSaleId())) {
                UserInfo saleInfo = userInfoRepository.get(userInfo.getUserSaleId());

                if (saleInfo != null) {
                    UserSaleVo userSaleVo = BeanUtil.copyProperties(saleInfo, UserSaleVo.class);
                    user.setUserSale(userSaleVo);
                }
            }
        }
        ...
        //默认收货地址
        QueryWrapper<UserDeliveryAddress> userDeliveryAddressQueryWrapper = new QueryWrapper<>();
        userDeliveryAddressQueryWrapper.eq("user_id", userId);
        userDeliveryAddressQueryWrapper.eq("ud_is_default", 1);
        UserDeliveryAddress deliveryAddress = deliveryAddressRepository.findOne(userDeliveryAddressQueryWrapper);

        if (deliveryAddress != null) {
            List<Integer> districtList = new ArrayList<>();
            districtList.add(deliveryAddress.getUdProvinceId());
            districtList.add(deliveryAddress.getUdCityId());
            districtList.add(deliveryAddress.getUdCountyId());

            user.setDistrictList(districtList);
        }
...
//            }
    ...
    public boolean saveCertificate(UserInfo userInfo) {
        ...
        if (!Objects.equals(userIsAuthentication, StateCode.USER_CERTIFICATION_VERIFY)) {
     ...
     */
    @Override
    public boolean addSourceUserId(Integer userId, Integer userParentId, Integer activityId, Boolean isRegister) {
        return true;
    }
    ...
    @Override
    @Transactional
    public boolean checkSourceUserId(Integer user_id) {
        // 分销员判断
        Integer sourceUserId = getSourceUserId();
        ...
        if (CheckUtil.isNotEmpty(sourceUserId)) {
            ...
            if (UserBase != null) {
                addSourceUserId(user_id, sourceUserId, 0, false);
            }
        }
