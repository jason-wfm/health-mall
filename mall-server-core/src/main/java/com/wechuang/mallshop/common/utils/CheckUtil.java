...
public class CheckUtil {
     ...
     * @param queryWrapper
     */
    public static <T> void handleFindInSet(List<?> params, String fieldName, QueryWrapper<T> queryWrapper) {
        if (CollectionUtils.isEmpty(params) || StringUtils.isBlank(fieldName) || queryWrapper == null) {
            return;
        }
        ...
        queryWrapper.and(wrapper -> {
            for (int i = 0; i < params.size(); i++) {
                ...
                // 手动拼接字段名到SQL中，只对参数使用占位符
                if (i == 0) {
                    wrapper.apply("FIND_IN_SET({0}, `" + fieldName + "`) > 0", param);
                } else {
                    wrapper.or().apply("FIND_IN_SET({0}, `" + fieldName + "`) > 0", param);
                }
            }
        });
    }
