...
import com.wechuang.mallshop.common.auth.Auth;
...
import com.wechuang.mallshop.common.pojo.dto.PayType;
...
import com.wechuang.mallshop.pay.model.input.PaymentInput;
import com.wechuang.mallshop.pay.model.output.PayOutput;
...
import com.wechuang.mallshop.pay.model.res.*;
import com.wechuang.mallshop.pay.model.vo.*;
...
public class PaymentIndexController extends BaseController {
    ...
    @Autowired
    private UserPayService uerPayService;
    ...
    public CommonRes<?> money(@Auth ContextUser user, PaymentReq req) {
        ContextUser contextUser = ContextUtil.getLoginUser();
        ...
        if (CheckUtil.isNotEmpty(req.getPmPoints())) {
            payInfo.setPmPoints(req.getPmPoints());
            payInfo.setPaymentMetId(StateCode.PAYMENT_MET_POINTS);
            req.setPaymentChannelId(StateCode.PAYMENT_CHANNEL_POINTS);
        }

        if (CheckUtil.isNotEmpty(req.getPmMoney())) {
            payInfo.setPaymentMetId(StateCode.PAYMENT_MET_MONEY);
            req.setPaymentChannelId(StateCode.PAYMENT_CHANNEL_MONEY);
        }
        ...
        if (userResourceBuy.getUserMoney().compareTo(payInfo.getPmMoney()) >= 0 || CheckUtil.isNotEmpty(payInfo.getPmPoints())) {

            uerPayService.checkPayPasswd(user.getUserId(), req.getPassword());
    ...
    public CommonRes<?> offlinePay(@Auth ContextUser user, PaymentReq req) {
        ...
        req.setPaymentChannelId(StateCode.PAYMENT_CHANNEL_OFFLINE);
    ...
    @Operation(summary = "银行打款接口", description = "银行打款接口")
    @RequestMapping(value = "/bankPay", method = RequestMethod.POST)
    public CommonRes<?> bankPay(@Auth ContextUser user, PaymentReq req) {
        Integer userId = ContextUtil.checkLoginUserId();
        boolean result = consumeDepositService.bankPay(req);

        return success(result);
    }

    @Operation(summary = "信用支付", description = "信用支付")
    @RequestMapping(value = "/creditPay", method = RequestMethod.POST)
    public CommonRes<?> creditPay(@Auth ContextUser user, PaymentReq req) {
        ...
        try {
            ...
            if (status == 200) {
                ...
                for (int i = 0; i < dataArray.size(); i++) {
                    JSONObject encryptObject = dataArray.getJSONObject(i);
                    String currentSerialNo = encryptObject.getStr("serial_no");

                    // 匹配响应头中的 serialNumber
                    if (serialNumber != null && serialNumber.equals(currentSerialNo)) {
                        ...
                        logger.info("匹配平台证书序列号: {} serialNo: {}", platSerialNo, currentSerialNo);
                        break;
                    }
                }
            }
    ...
    public CommonRes<WechatAppPayRes> wechatAppPay(PaymentReq req) {
        ...
        // 微信支付公钥id
        String publicKeyId = configBaseService.getConfig("wechat_pay_public_key_id");
        // 微信支付公钥
        String publicKeyPath = configBaseService.getConfig("wechat_pay_public_key");
        ...
        try {
            ...
            // 根据微信支付公钥验证签名结果
            boolean verifySignature = WxPayKit.verifyPublicKeySignature(response, publicKeyPath);
    ...
    @ResponseBody
    public void Paypal(HttpServletResponse response, PaymentReq req) {
        try {
            req.setPaymentChannelId(StateCode.PAYMENT_CHANNEL_PAYPAL);
            PayOutput output = getPayResult(req);
            PayPalVo payPalVo = configBaseService.getPayPalVo();
            //PayPalApiConfig payPalVo = getConfig();
            ...
            */
            OrderVo detail = orderService.detail(output.getTradeNo());
            Integer currency_id = detail.getCurrencyId();
            CurrencyBase currencyBase = currencyBaseRepository.get(currency_id);
            String currency_code = currencyBase.getCurrencySymbolRight();
            ...
            itemMap.put("invoice_id", output.getTradeNo());

            //output 设置订单金额 单位为分且最小为1
            Map<String, Object> amount = new HashMap<>();
            //todo 货币汇率处理
            amount.put("currency_code", currency_code);
            amount.put("value", detail.getOrderPaymentAmount());

            Map<String, Object> breakdownMap = new HashMap<>();
            HashMap<String, Object> exchangeRate = new HashMap<>();
            exchangeRate.put("currency_code", currency_code);
            exchangeRate.put("value", detail.getOrderPaymentAmount().subtract(detail.getOrderShippingFee()));
            breakdownMap.put("item_total", exchangeRate);

            HashMap<String, Object> shipping = new HashMap<>();
            shipping.put("currency_code", currency_code);
            shipping.put("value", detail.getOrderShippingFee());
            breakdownMap.put("shipping", shipping);

            HashMap<String, Object> tax_total = new HashMap<>();
            tax_total.put("currency_code", currency_code);
            tax_total.put("value", 0);
            breakdownMap.put("tax_total", tax_total);

            amount.put("breakdown", breakdownMap);

            itemMap.put("amount", amount);

            //商品SKU详情 - 根据订单好读取order-item
            ...
            unitAmount.put("value", output.getAmount());
            sku.put("unit_amount", unitAmount);
            sku.put("quantity", 1);
            sku.put("sku", "Order");
            sku.put("name", "Buy Online");
            items.add(sku);
             */
            ...
            for (OrderItem item : orderItems) {
                Map<String, Object> sku = new HashMap<>();
                sku.put("name", item.getProductItemName());
                sku.put("description", item.getOrderItemNote());

                //output 设置订单金额 单位为分且最小为1
                Map<String, Object> unitAmount = new HashMap<>();
                unitAmount.put("currency_code", currency_code);
                unitAmount.put("value", item.getOrderItemAmount());
                sku.put("unit_amount", unitAmount);
                sku.put("quantity", item.getOrderItemQuantity());
                sku.put("sku", item.getItemId());
    ...
    @RequestMapping(value = "/unipayNative", method = {RequestMethod.POST, RequestMethod.GET})
    @ResponseBody
    public CommonRes<UnionPayRes> unipayNaivePay(HttpServletRequest request, PaymentReq req) {
        UnionPayVo unionPayVo = configBaseService.getUnionPayVo();
        req.setPaymentChannelId(StateCode.PAYMENT_CHANNEL_UNION);

        PayOutput output = getPayResult(req);
        UnionPayRes unionPayRes = new UnionPayRes();
        unionPayRes.setOrderId(req.getOrderId());
        unionPayRes.setPaymentAmount(Convert.toBigDecimal(output.getAmount()).divide(BigDecimal.valueOf(100)));

        String body = ObjectUtil.defaultIfBlank(output.getTitle(), output.getTradeNo());


        try {
            String notifyUrl = unionPayVo.getNotifyUrl();

            String ip = IJPayUtil.getRealIp(request);
            if (StrUtil.isBlank(ip)) {
                ip = "127.0.0.1";
            }

            com.ijpay.unionpay.model.UnifiedOrderModel.UnifiedOrderModelBuilder builder = com.ijpay.unionpay.model.UnifiedOrderModel.builder();

            Map<String, String> params = builder
                    .service(ServiceEnum.NATIVE.toString())
                    .mch_id(unionPayVo.getMachId())
                    .out_trade_no(output.getTradeNo())
                    .body(body)
                    .attach("聚合支付 SDK")
                    .total_fee(Convert.toStr(output.getAmount()))
                    .mch_create_ip(ip)
                    .notify_url(notifyUrl)
                    .nonce_str(WxPayKit.generateStr())
                    .build()
                    .createSign(unionPayVo.getKey(), SignType.MD5);

            String xmlResult = UnionPayApi.execution(unionPayVo.getServerUrl(), params);
            logger.info("xmlResult:" + xmlResult);
            Map<String, String> result = WxPayKit.xmlToMap(xmlResult);
            unionPayRes.setData(result);
            return success(unionPayRes);
        } catch (Exception e) {
            e.printStackTrace();
            return fail(e.getMessage(), unionPayRes);
        }
    }
    ...
    @ResponseBody
    public CommonRes<?> unionJsPay(HttpServletRequest request, PaymentReq req) {
        ContextUser contextUser = ContextUtil.getLoginUser();
        ...
        try {
            ...
            Map<String, String> params = builder
                    ...
                    .mch_id(unionPayVo.getMachId())
                    .out_trade_no(output.getTradeNo())
                    .body(output.getTitle())
                    .user_id(Convert.toStr(userId))
                    .customer_ip(ip)
                    .attach("聚合支付 SDK")
                    .total_fee(Convert.toStr(output.getAmount()))
                    .mch_create_ip(ip)
    ...
    @Operation(summary = "销售员支付", description = "销售员支付")
    @RequestMapping(value = "/saleOffline", method = RequestMethod.POST)
    public CommonRes<?> saleOffline(ConsumeDepositOfflinePayReq req) {
