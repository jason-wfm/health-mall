...
public class PaymentCallbackUnionpayController extends BaseController {
    @Autowired
    ...
    private static final Logger logger = LoggerFactory.getLogger(PaymentCallbackUnionpayController.class);

    @RequestMapping(value = "/unionpayReturnUrl")
     ...
     * 异步通知
     */
    @RequestMapping(value = "/unionpayNotify", method = {RequestMethod.POST, RequestMethod.GET})
    public String notifyUrl(HttpServletRequest request) {
        UnionPayVo unionPayVo = configBaseService.getUnionPayVo();
        try {
            boolean verifyResult = false;

            String xmlMsg = IJPayUtil.readData(request);
            ...
            if ("0".equals(status) && "0".equals(returnCode)) {
                ...
                if (WxPayKit.verifyNotify(params, unionPayVo.getKey(), SignType.MD5)) {
                    logger.info("支付成功....");
                    verifyResult = true;
                    // 更新订单信息
                    // 发送通知等
