...
import com.wechuang.mallshop.pay.model.req.ConsumeRecordListReq;
 ...
 * @since 2021-06-30
 */
@Tag(name = "交易明细表-账户收支明细-资金流水表-账户金额变化流水")
@RestController
@RequestMapping("/manage/pay/consumeRecord")
public class ConsumeRecordController extends BaseController {
    @Autowired
    private ConsumeRecordService consumeRecordService;

    @PreAuthorize("hasAuthority('/manage/pay/consumeRecord/list')")
    @Operation(summary = "交易明细表-账户收支明细-资金流水表-账户金额变化流水-分页列表查询", description = "交易明细表-账户收支明细-资金流水表-账户金额变化流水-分页列表查询")
    ...
    public CommonRes<BaseListRes<ConsumeRecord>> list(ConsumeRecordListReq consumeRecordListReq) {
        ...
        if (loginUser != null) {
            ...
            if (CheckUtil.isNotEmpty(chainId)) {
                consumeRecordListReq.setChainId(chainId);
            }
        }
