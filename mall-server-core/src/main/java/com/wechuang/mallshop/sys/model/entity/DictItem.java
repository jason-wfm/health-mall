...
@Schema(name = "DictItem对象", description = "字典项表")
...
public class DictItem implements Serializable {
    ...
    @Schema(description = "配置类型(ENUM):readonly-只读文本;number-数字;text-单行文本;textarea-多行文本;array-数组;password-密码;radio-单选框;checkbox-复选框;select-下拉框;icon-字体图标;date-日期;datetime-时间;image-单张图片;images-多张图片;file-单个文件;files-多个文件;ueditor-富文本编辑器;area-地区选择")
    @TableField("dict_item_datatype")
    private String dictItemDatatype;

    @Schema(description = "是否必须(BOOL):0-非必填;1-必填")
    @TableField("dict_item_required")
    private Boolean dictItemRequired;

    @Schema(description = "placeholder")
    @TableField("dict_item_placeholder")
    ...
    @TableField("dict_item_enable")
    private Boolean dictItemEnable;

    @Schema(description = "系统内置(BOOL):1-是; 0-否")
    @TableField("dict_item_buildin")
    private Boolean dictItemBuildin;
