...
import com.wechuang.mallshop.sys.model.vo.WxTelMsgPushVo;
import com.wechuang.mallshop.sys.repository.MessageTemplateRepository;
import com.wechuang.mallshop.sys.repository.WechatTplmsgRepository;
        ...
        if (template != null) {
            ...
            if (template.getMessageSmsEnable() && !userSetting.isEmpty() && activeProfile.equals("prod")) {
                ...
                if (StrUtil.isNotEmpty(mobile)) {
                    ...
                    smsDto.setTemplateCode(template.getMessageTplId());
                    smsDto.setContent(template.getMessageSms());
                    smsDto.setParamMap(args); //传入模板对应参数

                    smsDto.setTengxunTemplateId(Convert.toInt(template.getMessageTplId()));
