...
import com.wechuang.mallshop.sys.dao.ContractTypeDao;
 ...
 */
@Repository
public class ContractTypeRepositoryImpl extends BaseRepositoryImpl<ContractTypeDao, ContractType> implements ContractTypeRepository {
    @Autowired
    private LangMetaRepository langMetaRepository;

    @Override
    public List<ContractType> gets(Collection<? extends Serializable> a) {
        List<ContractType> list = super.gets(a);
        ...
        for (ContractType it : list) {
            if (ObjectUtil.isNotEmpty(it)) {
                it.setContractTypeName(langMetaRepository.getTranslate(it.getContractTypeName(), to, ConstantConfig.BASE_LANG, "sys_contract_type", Convert.toStr(it.getContractTypeName()), "contract_type_name", 0));
            }
        }
