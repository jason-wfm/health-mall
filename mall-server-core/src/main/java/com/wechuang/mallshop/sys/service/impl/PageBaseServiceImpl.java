...
import com.wechuang.mallshop.cms.repository.ArticleBaseRepository;
import com.wechuang.mallshop.cms.repository.ArticleCategoryRepository;
...
import com.wechuang.mallshop.marketing.model.vo.PageDataVo;
...
import com.wechuang.mallshop.sys.model.req.PageDataReq;
...
import com.wechuang.mallshop.sys.repository.PageBaseRepository;
...
//                idList.add(22L);
            ...
            case 18:
                // 弹窗活动（页面装修数据源）
                QueryWrapper<ActivityBase> popActivityQuery = new QueryWrapper<>();
                if (StrUtil.isNotEmpty(pageDataReq.getName())) {
                    popActivityQuery.like("activity_name", pageDataReq.getName());
                }
                popActivityQuery.eq("activity_state", StateCode.ACTIVITY_STATE_NORMAL)
                        .eq("activity_type_id", StateCode.ACTIVITY_TYPE_POP)
                        .orderByDesc("activity_id");
                IPage<ActivityBase> popActivityPage = activityBaseRepository.lists(popActivityQuery, pageDataReq.getPage(), pageDataReq.getSize());
                if (CollectionUtil.isNotEmpty(popActivityPage.getRecords())) {
                    popActivityPage.getRecords().forEach(item -> {
                        if (StrUtil.isNotEmpty(item.getActivityRule())) {
                            ActivityRuleVo ruleVo = JSONUtil.parseObject(item.getActivityRule(), ActivityRuleVo.class);
                            if (ruleVo != null && ruleVo.getPopup() != null) {
                                PopupVo pop = ruleVo.getPopup();
                                PageDataItemVo pageDataItemVo = new PageDataItemVo();
                                pageDataItemVo.setId(item.getActivityId().longValue());
                                pageDataItemVo.setName(item.getActivityName());
                                pageDataItemVo.setPath(pop.getPopUpImage());
                                pageDataItemVo.setProductTips(StrUtil.blankToDefault(pop.getPopUpUrl(), ""));
                                pageDataItemVoList.add(pageDataItemVo);
                            }
                        }
                    });
    ...
    private void fixData(PageBaseRes pageBaseRes) {
        ...
        if (CollUtil.isNotEmpty(item_id_row)) {
            ...
            //List<Map> activityItemList = activityItemRepository.fixActivityItemData(activityItemList);
                ...
                if (ObjectUtil.equal(eltmType, 4)) {
                    ...
                    for (Object item : data) {
                        ...
                        if (product_item_opl.isPresent()) {
                            ...
                            Integer activity_type_id = 0;

                            Optional<ActivityItem> activity_item_row_opl = activityItemList.stream().filter(s -> ObjectUtil.equal(s.getItemId(), did)).findFirst();
                            if (activity_item_row_opl.isPresent()) {
                                ActivityItem activity_item_row = activity_item_row_opl.get();
                                activity_type_id = activity_item_row.getActivityTypeId();
                            }

                            //秒杀
//                            if (activity_type_id.intValue() == StateCode.ACTIVITY_TYPE_LIMITED_DISCOUNT)
//                            {
//                            }
...
//                    ((JSONObject) page_code_row).set("eltm13",eltm13);
                    ...
                    JSONObject eltm16 = (JSONObject) ((JSONObject) page_code_row).get("eltm16");
                    if (eltm16 == null) continue;
                    ...
                    for (Object item : data) {
                        Long did = Convert.toLong(((JSONObject) item).get("did"));
                        Optional<ActivityItem> product_item_opl = activityItemList.stream().filter(s -> ObjectUtil.equal(s.getItemId(), did)).findFirst();
                        if (product_item_opl.isPresent()) {
                            ActivityItem product_item_row = product_item_opl.get();
                            BigDecimal itemSalePrice = Convert.toBigDecimal(((JSONObject) item).get("ItemSalePrice"));
                            BigDecimal activity_item_price = product_item_row.getActivityItemPrice();

                            Integer selectType = Convert.toInt(((JSONObject) item).get("selectType"));

                            //修正最新商品价格
                            if (selectType.intValue() == 14) {
                                ((JSONObject) item).set("ItemSalePrice", activity_item_price);
                            }
                        }
                    }
        ...
        if (to != null) {
            for (Object page_code_row : pageCodeRows) {
                Integer eltmType = Convert.toInt(((JSONObject) page_code_row).get("eltmType"));
                String eltmName = String.format("eltm%s", eltmType);

                JSONObject eltmObj = (JSONObject) ((JSONObject) page_code_row).get(eltmName);
                if (eltmObj == null) continue;

                String title = Convert.toStr(eltmObj.get("title"));
                if (CheckUtil.isNotEmpty(title)) {
                    eltmObj.set("title", langMetaRepository.getTranslate(title, to, ConstantConfig.BASE_LANG, "diy_page", "diy_page", null, 0));
                }

                String linkText = Convert.toStr(eltmObj.get("linkText"));
                if (CheckUtil.isNotEmpty(linkText)) {
                    eltmObj.set("linkText", langMetaRepository.getTranslate(linkText, to, ConstantConfig.BASE_LANG, "diy_page", "diy_page", null, 0));
                }


                Object obj = eltmObj.get("data");
                    ...
                    for (Object item : data) {
                        ...
                        if (nameObj != null) {
                            if (!StringUtils.isEmpty(nameObj.toString())) {
                                ...
                                ((JSONObject) item).set("name", langMetaRepository.getTranslate(name, to, ConstantConfig.BASE_LANG, "diy_page", "diy_page", null, 0));
                            }
                        }


                        Object productTipsObj = ((JSONObject) item).get("ProductTips");
                        if (productTipsObj != null) {
                            if (!StringUtils.isEmpty(productTipsObj.toString())) {
                                String productTips = productTipsObj.toString();

                                ((JSONObject) item).set("ProductTips", langMetaRepository.getTranslate(productTips, to, ConstantConfig.BASE_LANG, "diy_page", "diy_page", null, 0));
                            }
                        }
                    }
                }
    ...
    public Map getAllCenterMenu() {
        ...
        menu14.setIcon("zc zc-tuiguangzhongxin");
        menu14.setFeatureKey("fenxiao");
        menu14.setUrl("/member/fans/index");


        PageMenuVo menu24 = new PageMenuVo();
        list.add(menu24);
        menu24.setId(13);
        menu24.setName("学习中心");
        menu24.setIsShow(configBaseService.ifEdu());
        menu24.setCat(1);
        menu24.setColor("#95de64");
        menu24.setIcon("zc zc-xuexizhongxin");
        menu24.setFeatureKey("CourseStudy");
        menu24.setUrl("/edu/course/study");

        PageMenuVo menu28 = new PageMenuVo();
        list.add(menu28);
        menu28.setId(14);
        menu28.setName("我的订阅");
        menu28.setIsShow(configBaseService.ifEdu());
        menu28.setCat(1);
        menu28.setColor("#69c0ff");
        menu28.setIcon("zc zc-dingyuezhongxin");
        menu28.setFeatureKey("LecturerSubscribe");
        menu28.setUrl("/edu/lecturer/subscribe");

        PageMenuVo menu1 = new PageMenuVo();
        ...
        menu1.setCat(1);
        menu1.setColor("#DB384C");
        menu1.setIcon("zc zc-wodepintuan");
        menu1.setFeatureKey("FightGrp");
        menu1.setUrl("/activity/fightgroup/order");
/*
...
*/
        ...
        PageMenuVo menu5 = new PageMenuVo();
        list.add(menu5);
        menu5.setId(4);
        menu5.setName("我的砍价");
        menu5.setIsShow(true);
        menu5.setCat(1);
        menu5.setColor("#b37feb");
        menu5.setIcon("zc zc-wodekanjia");
        menu5.setFeatureKey("CutPrice");
        menu5.setUrl("/activity/cutprice/userlist");
...
*/
        ...
        PageMenuVo menu30 = new PageMenuVo();
        list.add(menu30);
        menu30.setId(30);
        menu30.setName("平台客服");
        menu30.setIsShow(true);
        menu30.setCat(1);
        menu30.setColor("#399efd");
        menu30.setIcon("zc zc-kefu2");
        menu30.setFeatureKey("KefuKey");
        menu30.setUrl("");

        PageMenuVo menu51 = new PageMenuVo();
        list.add(menu51);
        menu51.setId(51);
        menu51.setName("清除缓存");
        menu51.setIsShow(true);
        menu51.setCat(1);
        menu51.setColor("#80b7f2");
        menu51.setIcon("zc zc-qingchuhuancun");
        menu51.setFeatureKey("CleanCacheKey");
        menu51.setUrl("");
    ...
    public Map getUserCenterMenu() {
        ...
        if (CheckUtil.isNotEmpty(appMemberCenter)) {
            ...
            for (Map it : pageMenuVoList) {
                ...
                if (it.get("FeatureKey").equals("Credit")) {
                    it.put("isShow", false);
                }
            }
        } else {
            ...
            for (PageMenuVo it : pageMenuVoList) {
                it.setName(__(it.getName()));
                //it.put("name", __(it.get("name").toString()));
                if (it.getFeatureKey().equals("Credit")) {
                    it.setIsShow(false);
                }
            }
        }
