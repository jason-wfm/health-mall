...
import com.wechuang.mallshop.common.consts.ConstantSys;
...
import com.wechuang.mallshop.pay.model.vo.UnionPayVo;
...
import com.wechuang.mallshop.sys.model.req.ConfigBaseIndexReq;
import com.wechuang.mallshop.sys.model.req.ConfigBaseListReq;
import com.wechuang.mallshop.sys.model.res.ConfigBaseIndexRes;
import com.wechuang.mallshop.sys.model.res.ConfigListRes;
import com.wechuang.mallshop.sys.model.vo.LangItemVo;
import com.wechuang.mallshop.sys.model.vo.LangOptVo;
...
import static com.wechuang.mallshop.common.utils.UploadUtil.getUploadFileDir;
...
public class ConfigBaseServiceImpl extends BaseServiceImpl<ConfigBaseRepository, ConfigBase, ConfigBaseListReq> implements ConfigBaseService {
    ...
    public boolean edit(ConfigBase data) {
        ...
        if (data.getConfigKey().equals("money_pay_enable")
                ...
                || data.getConfigKey().equals("paypal_enable")
    ...
    public boolean editSite(Map<String, Object> objectMap) {
        ...
        if (!objectMap.isEmpty()) {
            if (objectMap.containsKey("withdraw_received_day") && objectMap.containsKey("order_return_hidden_time")) {
                BigDecimal withdrawReceivedDay = Convert.toBigDecimal(objectMap.get("withdraw_received_day"), BigDecimal.valueOf(7));
                BigDecimal orderReturnHiddenTime = Convert.toBigDecimal(objectMap.get("order_return_hidden_time"), BigDecimal.valueOf(7));

                if (withdrawReceivedDay.compareTo(orderReturnHiddenTime) <= 0) {
                    throw new BusinessException(__("订单可提现时间需大于退货按钮隐藏时间！"));
                }
            } else {
                if (objectMap.containsKey("withdraw_received_day")) {
                    BigDecimal withdrawReceivedDay = Convert.toBigDecimal(objectMap.get("withdraw_received_day"));
                    BigDecimal orderReturnHiddenTime = getConfig("order_return_hidden_time", BigDecimal.valueOf(7));
                ...
                if (objectMap.containsKey("order_return_hidden_time")) {
                    BigDecimal orderReturnHiddenTime = Convert.toBigDecimal(objectMap.get("order_return_hidden_time"));
                    BigDecimal withdrawReceivedDay = getConfig("withdraw_received_day", BigDecimal.valueOf(7));

                    if (orderReturnHiddenTime.compareTo(withdrawReceivedDay) >= 0) {
                        throw new BusinessException(__("退货按钮隐藏时间需小于订单可提现时间！"));
                    }
                }
            }
    ...
    public Boolean init() {
        ...
        currencyBaseRepository.initCurrencyRateMap();
    ...
    public WxPayV3Vo getWxPayV3Vo() {
        ...
        String publicKey = getConfig("wechat_pay_public_key");
        wxPayV3Vo.setPublicKeyPath(publicKey);

        String publicKeyId = getConfig("wechat_pay_public_key_id");
        wxPayV3Vo.setPublicKeyId(publicKeyId);
     ...
     */
    @Override
    public PayPalVo getPayPalVo() {
        PayPalVo payPalVo = new PayPalVo();

        // AppId
        String paypalAppId = getConfig("paypal_client_id");
        payPalVo.setClientId(paypalAppId);

        // 私钥
        String paypalSecret = getConfig("paypal_client_secret");
        payPalVo.setSecret(paypalSecret);
     ...
     */
    @Override
    public UnionPayVo getUnionPayVo() {
        UnionPayVo unionPayVo = new UnionPayVo();

        //
        String paypalAppId = getConfig("unionpay_mach_id");
        unionPayVo.setMachId(paypalAppId);

        // 私钥
        String paypalSecret = getConfig("unionpay_key");
        unionPayVo.setKey(paypalSecret);
    ...
    public List<SelectVo> initPaymentChannelList() {
        ...
        for (ConfigType configType : configTypes) {
            try {
                ...
                //1434	paypal支付 paypal_enable

                String id = "";
                String ck = "";
                String img = "";

                if (Objects.equals(configType.getConfigTypeId(), StateCode.PAYMENT_CHANNEL_WECHAT)) {
                    id = "wxpay";
                    ck = "wechat_pay_enable";
                    img = "wechat_pay_logo";
                } else if (Objects.equals(configType.getConfigTypeId(), StateCode.PAYMENT_CHANNEL_ALIPAY)) {
                    id = "alipay";
                    ck = "alipay_enable";
                    img = "alipay_logo";
                } else if (Objects.equals(configType.getConfigTypeId(), StateCode.PAYMENT_CHANNEL_OFFLINE)) {
                    id = "offline";
                    ck = "offline_pay_enable";
                    img = "offline_pay_logo";
                } else if (Objects.equals(configType.getConfigTypeId(), StateCode.PAYMENT_CHANNEL_MONEY)) {
                    id = "money";
                    ck = "money_pay_enable";
                    img = "money_pay_logo";
                } else if (Objects.equals(configType.getConfigTypeId(), StateCode.PAYMENT_CHANNEL_POINTS)) {
                    continue;
                } else if (Objects.equals(configType.getConfigTypeId(), StateCode.PAYMENT_CHANNEL_PAYPAL)) {
                    id = "paypal";
                    ck = "paypal_enable";
                    img = "paypal_logo";
                } else if (Objects.equals(configType.getConfigTypeId(), StateCode.PAYMENT_CHANNEL_BANK)) {
                    id = "bank";
                    ck = "bank_pay_enable";
                    img = "bank_pay_logo";
                } else if (Objects.equals(configType.getConfigTypeId(), StateCode.PAYMENT_CHANNEL_CREDIT)) {
                    continue;
                } else {
    ...
    public Map<String, Object> getSiteInfo(String sourceLang) {
        ...
        //国际区号处理
        List<Map> intlOpt = new ArrayList<>();
        if (ifMultilang()) {
            List<CurrencyBase> currencyBaseList = currencyBaseRepository.find(new LambdaQueryWrapper<CurrencyBase>().eq(CurrencyBase::getCurrencyStatus, true).orderByAsc(CurrencyBase::getCurrencySort));
            for (CurrencyBase item : currencyBaseList) {
                Map<String, Object> opt = new HashMap<>();
                opt.put("label", item.getCurrencyTitle() + "+" + item.getCurrencyId());
                opt.put("value", "+" + item.getCurrencyId());
    ...
    public Map<String, Object> listTranslateLang() {
        Integer currencyId = 86;
        try {
            currencyId = Convert.toInt(getParameter("currency_id", 86));
        } catch (Exception e) {
        }
        ...
        //查询出启用的语言
        LangItemVo df = new LangItemVo();
        LangItemVo selectLang = null;
        List<LangItemVo> items = new ArrayList<>();

        //设置默认值
        df.setCurrencyId(86);
        df.setImg("https://static.shopsuite.cn/shopsuite/flags/square/5322affbc4d56.png");
        df.setLabel("中国");
        df.setLang("zh-CN");
        df.setStandard(false);
        df.setSymbol("¥");
        df.setSymbolRight("CNY");

        LangOptVo opt;
        opt = BeanUtil.copyProperties(df, LangOptVo.class);
        opt.getItems().add(df);
        out.put("opt", opt);
        ...
        if (ifMultilang()) {
            List<CurrencyBase> currencyBaseList = currencyBaseRepository.find(new LambdaQueryWrapper<CurrencyBase>().eq(CurrencyBase::getCurrencyStatus, true).orderByAsc(CurrencyBase::getCurrencySort));
            for (CurrencyBase item : currencyBaseList) {
                LangItemVo langItemVo = new LangItemVo();
                langItemVo.setLang(item.getCurrencyLang());
                langItemVo.setCurrencyId(item.getCurrencyId());
                langItemVo.setSymbol(item.getCurrencySymbolLeft());
                langItemVo.setSymbolRight(item.getCurrencySymbolRight());
                langItemVo.setLabel(item.getCurrencyTitle());
                langItemVo.setStandard(item.getCurrencyIsStandard());
                langItemVo.setImg(item.getCurrencyImg());

                items.add(langItemVo);

                if (item.getCurrencyDefaultLang()) {
                    df = langItemVo;
                }

                // 判断是否为默认语言
                if (item.getCurrencyId().equals(currencyId)) {
                    selectLang = langItemVo;
                }
            }

            if (ObjectUtil.isEmpty(df) && items.size() > 0) {
                df = items.get(0);
            }

            if (!ObjectUtil.isEmpty(selectLang)) {
                df = selectLang;
            }
            ...
            List<LangStandard> langStandards = langStandardRepository.find(new LambdaQueryWrapper<LangStandard>().eq(LangStandard::getFrontend, true));
            for (CurrencyBase cb : currencyBaseList) {
                if (CheckUtil.isEmpty(cb.getCurrencyLang())) {
                    continue;
                }

                if (cb.getCurrencyLang().equals("zh-CN")) {
                    continue;
                }

                String langKey = cb.getCurrencyLang().replace("-", "_");
                String mapKey = StringHelpers.underLineToCamel(langKey);

                if (cb.getCurrencyLang().equals("en-US")) {
                    mapKey = StringHelpers.underLineToCamel("en_GB");
                } else if (cb.getCurrencyLang().equals("es-ES")) {
                    mapKey = StringHelpers.underLineToCamel("es_MX");
                }

                Map<Object, Object> transMap = new HashMap<>();

                for (LangStandard item : langStandards) {
                    if (CheckUtil.isNotEmpty(item.getZhCN())) {
                        Map itemMap = Convert.toMap(String.class, Object.class, item);
                        transMap.put(itemMap.get("zhCN"), itemMap.get(mapKey));
                    }
                }

                lang.put(langKey, transMap);
            }
        }
    ...
    public boolean cleanCache() {
        ...
        Set<String> pc_layout_data_key = redisService.keys("pcLayoutData:*");
        Set<String> pc_help_key = redisService.keys("getPcHelp:*");
        ...
        redisService.del(pc_layout_data_key);
        redisService.del(pc_help_key);
    ...
    @Override
    public boolean ifIndustry() {
        return getConfig("industry_enable", false);
    }

    @Override
    public boolean ifBatchOrder() {
        return getConfig("batch_order_enable", false);
    }
    ...
    public boolean ifB2bHall() {
        return getConfig("hall_b2b_enable", false);
    }

    @Override
    public boolean ifHall() {
    ...
    @Override
    public Integer getOrderIsSync() {
        Integer orderIsSync = 3; //无需同步
        Integer apiOrderType = getConfig("api_order_type", 0);

        if (apiOrderType.intValue() == 0) {
            orderIsSync = 3;
        } else {
            orderIsSync = 0;
        }
    ...
    @Override
    public void cleanRequestCache() {
        List<String> keys = RequestContextVariable.getCacheKeyList();

        if (!org.apache.commons.collections4.CollectionUtils.isEmpty(keys)) {
            Long del = redisService.del(keys);
            logger.info("The number of keys that were removed:{}", del);
        }
