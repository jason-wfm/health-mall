...
import com.wechuang.mallshop.trade.model.input.OrderCommentInput;
...
import com.wechuang.mallshop.trade.repository.OrderCommentRepository;
...
public class OrderCommentServiceImpl extends BaseServiceImpl<OrderCommentRepository, OrderComment, OrderCommentListReq> implements OrderCommentService {
    ...
    public void addOrderComment(OrderCommentInput input) {
        ...
        orderComment.setCommentPoints(BigDecimal.ZERO);
        ...
        if (storeAnalytics != null) {
            BigDecimal totalDescCredit = NumberUtil.add(storeAnalytics.getStoreDesccredit(), input.getStoreDesccredit());
            BigDecimal totalServiceCredit = NumberUtil.add(storeAnalytics.getStoreServicecredit(), input.getStoreServicecredit());
            BigDecimal totalDeliveryCredit = NumberUtil.add(storeAnalytics.getStoreDeliverycredit(), input.getStoreDeliverycredit());
            int evaluationNum = storeAnalytics.getStoreEvaluationNum() + 1;

            storeAnalytics.setStoreDesccredit(totalDescCredit);
            storeAnalytics.setStoreServicecredit(totalServiceCredit);
            storeAnalytics.setStoreDeliverycredit(totalDeliveryCredit);
            storeAnalytics.setStoreEvaluationNum(evaluationNum);

            BigDecimal descAvg = NumberUtil.div(totalDescCredit, evaluationNum, 1);
            BigDecimal serviceAvg = NumberUtil.div(totalServiceCredit, evaluationNum, 1);
            BigDecimal deliveryAvg = NumberUtil.div(totalDeliveryCredit, evaluationNum, 1);
            storeAnalytics.setStoreEvaluationRate(NumberUtil.div(NumberUtil.add(descAvg, serviceAvg, deliveryAvg), 3, 1));
            ...
            storeAnalytics.setStoreDesccredit(input.getStoreDesccredit());
            storeAnalytics.setStoreServicecredit(input.getStoreServicecredit());
            storeAnalytics.setStoreDeliverycredit(input.getStoreDeliverycredit());
            storeAnalytics.setStoreEvaluationNum(1);
            storeAnalytics.setStoreEvaluationRate(NumberUtil.div(NumberUtil.add(input.getStoreDesccredit(), input.getStoreServicecredit(), input.getStoreDeliverycredit()), 3, 1));
