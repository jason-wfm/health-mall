...
@Tag(name = "购物车表")
@RestController
@RequestMapping("/manage/trade/userCart")
public class UserCartController extends BaseController {
    @Autowired
    private UserCartService userCartService;

    @PreAuthorize("hasAuthority('/manage/trade/userCart/list')")
    @Operation(summary = "购物车表-分页列表查询", description = "购物车表-分页列表查询")
    @RequestMapping(value = "/list", method = RequestMethod.GET)
    public CommonRes<BaseListRes<UserCart>> list(UserCartListReq userCartListReq) {
        IPage<UserCart> pageList = userCartService.lists(userCartListReq);
