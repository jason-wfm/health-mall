...
@Schema(description = "退款退货表-发货退货,卖家也可以决定不退货退款，买家申请退款不支持。卖家可以主动退款。分页查询")
public class OrderReturnListReq extends BaseListReq {
    ...
    @Schema(description = "退款完成时间-开始")
    @QueryField(value = "return_add_time", type = QueryType.GE)
    private Long returnAddStart;

    @Schema(description = "退款完成时间-结束")
    @QueryField(value = "return_add_time", type = QueryType.LE)
    private Long returnAddEnd;

    public OrderReturnListReq() {
        setSidx("return_add_time");
        setDesc();
    }
