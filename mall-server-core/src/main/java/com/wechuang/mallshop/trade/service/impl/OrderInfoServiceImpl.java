...
public class OrderInfoServiceImpl extends BaseServiceImpl<OrderInfoRepository, OrderInfo, OrderInfoListReq> implements OrderInfoService {
    ...
    @Override
    public boolean saveOrderInfo(String orderId, Integer orderIsSettlemented) {
        List<String> orderIds = Convert.toList(String.class, orderId);
        QueryWrapper<OrderInfo> queryWrapper = new QueryWrapper<>();
        queryWrapper.in("order_id", orderIds).eq("order_is_settlemented", 2);
    ...
    @Override
    public List<String> getReturnHiddenOrderIds() {
        QueryWrapper<OrderInfo> queryWrapper = new QueryWrapper<>();
        queryWrapper.in("order_state_id", StateCode.ORDER_STATE_RECEIVED, StateCode.ORDER_STATE_FINISH);

        Float orderReturnHiddenTime = configBaseService.getConfig("order_return_hidden_time", 7f);
        int second = NumberUtil.mul(orderReturnHiddenTime, 24, 60, 60).intValue();
        long time = DateUtil.offsetSecond(new Date(), -second).getTime();

        queryWrapper.lt("update_time", time);
        queryWrapper.eq("order_return_hidden", 0);

        return Convert.toList(String.class, findKey(queryWrapper));
    }

    @Override
    public List<String> getOrderShippingWaitPushWx() {
        QueryWrapper<OrderInfo> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("order_push_wx_shipping", 0);
        queryWrapper.in("order_state_id", StateCode.ORDER_STATE_SHIPPED, StateCode.ORDER_STATE_RECEIVED, StateCode.ORDER_STATE_FINISH);

        return Convert.toList(String.class, findKey(queryWrapper));
    }

    @Override
    public List<String> getInvoiceHiddenOrderIds() {
        QueryWrapper<OrderInfo> queryWrapper = new QueryWrapper<>();
        queryWrapper.in("order_state_id", StateCode.ORDER_STATE_RECEIVED, StateCode.ORDER_STATE_FINISH);

        Float orderInvoiceHiddenTime = configBaseService.getConfig("order_invoice_hidden_time", 7f);
        int second = NumberUtil.mul(orderInvoiceHiddenTime, 24, 60, 60).intValue();
        long time = DateUtil.offsetSecond(new Date(), -second).getTime();

        queryWrapper.lt("update_time", time);
        queryWrapper.eq("order_invoice_hidden", 0);

        return Convert.toList(String.class, findKey(queryWrapper));
    }

    @Override
    public List<String> getDistributionOrderIds() {
