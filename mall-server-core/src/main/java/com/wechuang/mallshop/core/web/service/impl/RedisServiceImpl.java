...
/**
 * Redis 操作实现（自研重写，基于 RedisTemplate 标准封装）；
 * 注意：本类不加 @Service，实例由 BaseRedisConfig.redisService() 工厂创建，避免同类型双 bean
 *
...
public class RedisServiceImpl implements RedisService {
    ...
    public Long lPushAll(String key, Object... values) {
        return redisTemplate.opsForList().rightPushAll(key, Arrays.asList(values));
    }

    @Override
    public Long lPushAll(String key, Long timeout, Object... values) {
        Long count = redisTemplate.opsForList().rightPushAll(key, Arrays.asList(values));
        expire(key, timeout);
        return count;
    }
    ...
    @Override
    public Long execute(String script, List<String> keys, String arg) {
        DefaultRedisScript<Long> redisScript = new DefaultRedisScript<>(script, Long.class);
        return redisTemplate.execute(redisScript, keys, arg);
    }
}
