...
@Schema(name = "StoreTransportType对象", description = "自定义物流运费及售卖区域类型表")
@JsonNaming(PropertyNamingStrategies.SnakeCaseStrategy.class)
public class StoreTransportType implements Serializable {
    private static final long serialVersionUID = 1L;

    @Schema(description = "物流及售卖区域编号")
    ...
    @Schema(description = "門店编号")
    @TableField("chain_id")
    private Integer chainId;

    @Schema(description = "计费规则(ENUM):1-按件数;2-按重量;3-按体积")
    ...
    @Schema(description = "全免运费(BOOL):0-不全免;1-全免（不限制地区且免运费）")
    @TableField("transport_type_free")
    private Boolean transportTypeFree;

    @Schema(description = "是否默认(BOOL):0-非默认;1-默认")
    @TableField("transport_is_default")
    private Boolean transportIsDefault;
