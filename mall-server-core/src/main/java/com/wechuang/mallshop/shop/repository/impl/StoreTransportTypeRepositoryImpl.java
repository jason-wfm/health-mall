...
import com.wechuang.mallshop.shop.dao.StoreTransportTypeDao;
...
public class StoreTransportTypeRepositoryImpl extends BaseRepositoryImpl<StoreTransportTypeDao, StoreTransportType> implements StoreTransportTypeRepository {
     ...
     */
    @Override
    public OrderFreightVo calFreight(Integer transport_type_id, Integer district_id, BigDecimal quantity, BigDecimal order_total, BigDecimal post_free_max) {
        ...
        if (storeTransportItemVo != null) {
            ...
            if (item != null) {
                ...
                if (transport_type_freight_free.compareTo(BigDecimal.ZERO) > 0 && ObjectUtil.compare(order_total, transport_type_freight_free) >= 0) {
                    ...
                    BigDecimal transport_item_default_num = item.getTransportItemDefaultNum();
                    BigDecimal transport_item_add_num = item.getTransportItemAddNum();
                    ...
                    if (CheckUtil.isNotEmpty(add_num) && CheckUtil.isNotEmpty(transport_item_add_num)) {
                        ...
                        BigDecimal sum = NumberUtil.add(transport_item_default_price, NumberUtil.mul(transport_item_add_price, add_num.divide(transport_item_add_num, 0, RoundingMode.UP)));
