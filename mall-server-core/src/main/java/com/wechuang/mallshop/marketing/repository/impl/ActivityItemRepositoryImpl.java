...
// +----------------------------------------------------------------------
package com.wechuang.mallshop.marketing.repository.impl;
...
import com.wechuang.mallshop.marketing.dao.ActivityItemDao;
 ...
 */
@Repository
public class ActivityItemRepositoryImpl extends BaseRepositoryImpl<ActivityItemDao, ActivityItem> implements ActivityItemRepository {
    @Autowired
    ...
    @Override
    public List<ActivityItem> gets(Collection<? extends Serializable> a) {
        List<ActivityItem> list = super.gets(a);
        ...
        if (multiCurrencyEnable) {
            ...
            for (ActivityItem it : list) {
                if (ObjectUtil.isNotEmpty(it)) {
                    //多语言汇率转换
                    it.setActivityItemPrice(currencyBaseRepository.getMultiCurrencyRate(it.getActivityItemPrice(), currencyLang));
                }
            }
        }
