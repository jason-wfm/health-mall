...
import com.wechuang.mallshop.marketing.dao.ActivityBaseDao;
...
public class ActivityBaseRepositoryImpl extends BaseRepositoryImpl<ActivityBaseDao, ActivityBase> implements ActivityBaseRepository {
    ...
    public Map getActivityItemNum(ActivityBase activityBase) {
        // 开源版营销活动仅保留优惠券与弹窗，不再从规则解析礼包/砍价等套餐 SKU 数量
        return new HashMap<>();
    }
    ...
    @Override
    public List<ActivityBase> gets(Collection<? extends Serializable> a) {
        List<ActivityBase> list = super.gets(a);
        ...
        for (ActivityBase it : list) {
            if (ObjectUtil.isNotEmpty(it)) {
                it.setActivityName(langMetaRepository.getTranslate(it.getActivityName(), to, ConstantConfig.BASE_LANG, "marketing_activity_base", Convert.toStr(it.getActivityName()), "activity_name", 0));
            }
        }
