...
import com.wechuang.mallshop.pt.dao.ProductAssistItemDao;
 ...
 */
@Repository
public class ProductAssistItemRepositoryImpl extends BaseRepositoryImpl<ProductAssistItemDao, ProductAssistItem> implements ProductAssistItemRepository {

    @Autowired
    private LangMetaRepository langMetaRepository;

    @Override
    public List<ProductAssistItem> gets(Collection<? extends Serializable> a) {
        List<ProductAssistItem> list = super.gets(a);
        ...
        for (ProductAssistItem it : list) {
            if (ObjectUtil.isNotEmpty(it)) {
                it.setAssistItemName(langMetaRepository.getTranslate(it.getAssistItemName(), to, ConstantConfig.BASE_LANG, "pt_product_assist_item", Convert.toStr(it.getAssistItemName()), "assist_item_name", 0));
            }
        }
