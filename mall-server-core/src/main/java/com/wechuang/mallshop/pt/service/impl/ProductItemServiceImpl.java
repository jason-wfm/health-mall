...
import com.wechuang.mallshop.pt.excel.ProductItemEditTemp;
import com.wechuang.mallshop.pt.excel.ProductItemEditTempListener;
...
public class ProductItemServiceImpl extends BaseServiceImpl<ProductItemRepository, ProductItem, ProductItemListReq> implements ProductItemService {
    ...
    public boolean batchEditStock(List<ProductEditStockInput> inputs) {
        ...
        for (ProductItem v : productItems) {
            ...
            if (input != null) {
                ...
                if (Objects.equals(input.getBillTypeId(), StateCode.BILL_TYPE_IN)) {
    ...
    public boolean editState(ProductItem item) {
        ...
        //修改产品索引表状态
        if (Objects.equals(itemEnable, StateCode.PRODUCT_STATE_OFF_THE_SHELF)) {
    ...
    @Override
    public ProductItemLevelDiscountRes levelDiscountList(Integer productId) {
        ...
        ProductItemLevelDiscountRes levelDiscountRes = new ProductItemLevelDiscountRes();
        ProductIndex productIndex = productIndexRepository.get(productId);
        ...
        if (!loginUser.isPlatform() && !CheckUtil.checkDataRights(loginUser.getStoreId(), productIndex, ProductIndex::getStoreId)) {
            throw new BusinessException(ResultCode.FORBIDDEN);
        }
        levelDiscountRes.setProductLevelMembership(productIndex.getProductLevelMembership());
        QueryWrapper<ProductItem> productItemQueryWrapper = new QueryWrapper<>();
        productItemQueryWrapper.eq("product_id", productId);
        ...
        List<ProductItemLevelDiscountVo> itemLevelDiscounts = new ArrayList<>();

        for (ProductItem productItem : productItems) {
            ProductItemLevelDiscountVo discountVo = new ProductItemLevelDiscountVo();
            discountVo.setItemId(productItem.getItemId());
            discountVo.setItemName(productItem.getItemName());
            discountVo.setItemUnitPrice(productItem.getItemUnitPrice());

            if (StrUtil.isNotEmpty(productItem.getItemLevelDiscount())) {
                String itemLevelDiscount = productItem.getItemLevelDiscount();
                Map discountMap = cn.hutool.json.JSONUtil.toBean(itemLevelDiscount, Map.class);
                discountVo.setLevelDiscount(discountMap);
            }
            itemLevelDiscounts.add(discountVo);
        }
        levelDiscountRes.setItemLevelDiscounts(itemLevelDiscounts);

        return levelDiscountRes;
    }

    @Override
    @Transactional
    public Boolean editLevelDiscounts(ProductItemLevelDiscountReq req) {
        ContextUser loginUser = getLoginUser();
        ...
        if (!req.getProductLevelMembership().equals(productIndex.getProductLevelMembership())) {
            productIndex.setProductLevelMembership(req.getProductLevelMembership());

            if (!productIndexRepository.edit(productIndex)) {
                throw new BusinessException(__("修改产品索引信息失败！"));
            }
        }
        String productItemDiscounts = req.getProductItemDiscounts();

        if (StrUtil.isNotEmpty(productItemDiscounts)) {
            List<UserLevel> userLevels = userLevelRepository.find(new QueryWrapper<>());

            if (CollectionUtil.isEmpty(userLevels)) {
                throw new BusinessException(__("用户等级列表不存在！"));
            }

            List<ProductItem> productItems = new ArrayList<>();
            List<Map> itemList = JSONUtil.parseArray(productItemDiscounts, Map.class);

            for (Map itemMap : itemList) {
                ProductItem productItem = new ProductItem();
                productItem.setItemId(Convert.toLong(itemMap.get("item_id")));
                Map levelDiscountMap = new HashMap();

                for (UserLevel userLevel : userLevels) {
                    String userLevelId = Convert.toStr(userLevel.getUserLevelId());
                    Integer levelDiscount = Convert.toInt(itemMap.get(userLevelId), 100);
                    levelDiscountMap.put(userLevelId, levelDiscount);
                }
                productItem.setItemLevelDiscount(JSONUtil.toJSONString(levelDiscountMap));

                productItems.add(productItem);
            }

            if (!productItemRepository.edit(productItems)) {
                throw new BusinessException(__("商品SKU等级折扣失败！"));
            }
        }
