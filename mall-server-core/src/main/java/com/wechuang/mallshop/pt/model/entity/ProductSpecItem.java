...
@TableName("pt_product_spec_item")
@Schema(name = "ProductSpecItem对象", description = "商品规格值表")
@JsonNaming(PropertyNamingStrategies.SnakeCaseStrategy.class)
public class ProductSpecItem implements Serializable {
    private static final long serialVersionUID = 1L;

    @Schema(description = "规格值编号")
    @TableId(value = "spec_item_id", type = IdType.AUTO)
    ...
    @TableField("spec_item_sort")
    private Integer specItemSort;

    @Schema(description = "是否启用(BOOL):0-不显示;1-显示")
    @TableField("spec_item_enable")
    private Boolean specItemEnable;
