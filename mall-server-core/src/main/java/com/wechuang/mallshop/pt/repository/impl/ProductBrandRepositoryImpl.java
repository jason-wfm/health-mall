...
import com.wechuang.mallshop.pt.dao.ProductBrandDao;
 ...
 */
@Repository
public class ProductBrandRepositoryImpl extends BaseRepositoryImpl<ProductBrandDao, ProductBrand> implements ProductBrandRepository {
    @Autowired
    ...
    public boolean remove(Collection<? extends Serializable> ids) {
        ...
        queryWrapper.in(ProductType::getBrandIds, ids);

        long count = productTypeRepository.count(queryWrapper);
        if (count > 0) {
            throw new BusinessException(String.format(__("有 %d 条类型使用，不可删除"), count));
        }
    ...
    @Override
    public List<ProductBrand> gets(Collection<? extends Serializable> a) {
        List<ProductBrand> list = super.gets(a);
        ...
        for (ProductBrand it : list) {
            if (ObjectUtil.isNotEmpty(it)) {
                it.setBrandName(langMetaRepository.getTranslate(it.getBrandName(), to, ConstantConfig.BASE_LANG, "pt_product_brand", Convert.toStr(it.getBrandId()), "brand_name", 0));
            }
        }
