...
import com.wechuang.mallshop.pt.dao.ProductSpecDao;
...
import com.wechuang.mallshop.pt.repository.ProductSpecItemRepository;
...
public class ProductSpecRepositoryImpl extends BaseRepositoryImpl<ProductSpecDao, ProductSpec> implements ProductSpecRepository {
    ...
    public List<ProductSpecOutput> getSpecs(String specIds) {
        ...
        specItemQueryWrapper.eq("spec_item_enable", true);

        ContextUser loginUser = ContextUtil.getLoginUser();

        if (loginUser != null) {
            specItemQueryWrapper.eq("store_id", loginUser.getStoreId());
        }
