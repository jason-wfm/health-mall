...
import com.wechuang.mallshop.pt.excel.ProductCommentListener;
import com.wechuang.mallshop.pt.excel.ProductCommentTemp;
import com.wechuang.mallshop.pt.model.entity.*;
import com.wechuang.mallshop.pt.model.req.ProductCommentListReq;
import com.wechuang.mallshop.pt.model.res.ProductCommentRes;
...
import com.wechuang.mallshop.trade.model.req.OrderCommentReq;
import com.wechuang.mallshop.trade.model.res.OrderCommentRes;
import com.wechuang.mallshop.trade.model.vo.EvaluationVo;
import com.wechuang.mallshop.trade.model.vo.OrderCommentVo;
...
import static com.wechuang.mallshop.common.api.StateCode.ORDER_ITEM_EVALUATION_YES;
...
public class ProductCommentServiceImpl extends BaseServiceImpl<ProductCommentRepository, ProductComment, ProductCommentListReq> implements ProductCommentService {
    ...
    public boolean editState(Long commentId, Boolean commentEnable) {
        ...
        if (!loginUser.isPlatform() && !CheckUtil.checkDataRights(loginUser.getStoreId(), productComment, ProductComment::getStoreId)) {
...
//            }
    ...
    private void addProductComment(ProductComment comment, Integer orderItemId) {
        ...
        if (Boolean.FALSE.equals(orderItemEvaluationStatus)) {
            // 获得积分 经验
            BigDecimal expEvaluateGood = configBaseService.getConfig("exp_evaluate_good", BigDecimal.ZERO);

            comment.setCommentPoints(BigDecimal.ZERO);
            if (ObjectUtil.compare(expEvaluateGood, BigDecimal.ZERO) > 0) {
                ExperienceVo experienceVo = new ExperienceVo();
                experienceVo.setUserId(orderItem.getUserId());
                experienceVo.setExp(expEvaluateGood);
                experienceVo.setExpTypeId(LevelCode.EXP_TYPE_EVALUATE_PRODUCT);
                experienceVo.setDesc(__("商品评论"));
    ...
    public void exportTemp(HttpServletResponse response) {
        ...
        try {
            ...
            WriteSheet writeSheet = EasyExcelUtil.writeSelectedSheet(ProductCommentTemp.class, 0, "导入虚拟评论模版");
            excelWriter.write(new ArrayList<ProductCommentTemp>(), writeSheet);
            excelWriter.finish();
    ...
    public void importTemp(MultipartFile file) throws Exception {
        AnalysisEventListener productCommentListener = new ProductCommentListener();
        Class<?> tempClass = ProductCommentTemp.class;

        InputStream inputStream = file.getInputStream();
        EasyExcel.read(inputStream)
                // 注册监听器，可以在这里校验字段
                .registerReadListener(productCommentListener)
                .head(tempClass)
                // 设置sheet,默认读取第一个
                .sheet()
                // 设置标题所在行数
